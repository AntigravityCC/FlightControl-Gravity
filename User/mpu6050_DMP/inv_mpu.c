...
#ifdef  DEBUG_ON_LOG
#define log_i(...)      do { printf("<<MPU6050--Log>@File:%s@Line:%d@FUNC:%s""", __FILE__, __LINE__, __FUNCTION__);printf(__VA_ARGS__);} while(0);
#define log_e(...)      do { printf("<<MPU6050--Log>@File:%s@Line:%d@FUNC:%s""", __FILE__, __LINE__, __FUNCTION__);printf(__VA_ARGS__);} while(0);
#else
